...
| Decades | Published separately in assets/collection_covers/decades/manifest.json, with its own schema. Two approved real-artwork parent-cover replacements are installed for publication; the 96 set assets are unchanged. Not included in runtime-lookup.json. | [Decades handover](artwork-categories/decades.md) |
...
| Other folders | Existing folders such as genre and holiday do not by themselves prove a category manifest or automatic builder support exists. | Inspect that category's files and consumers; document the actual state. |
