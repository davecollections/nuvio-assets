...
| Decades | Published separately in assets/collection_covers/decades/manifest.json, with its own schema. Both real-artwork parent-cover replacements are published and live-hash verified; the 96 set assets are unchanged. Not included in runtime-lookup.json. | [Decades handover](artwork-categories/decades.md) |
...
| Discover | Published in assets/collection_covers/discover/manifest.json with its own schema: 60 approved WebPs, all live-hash verified; 12 legacy JPGs retired after backup. Heroes have unique titles/images within each hero. General Builder support is not implemented. | [Discover handover](artwork-categories/discover.md) |
| Based on | Published forty approved v3 WebPs, all live-hash verified; stronger text-outline focus, separate title logos and distinct-image heroes. Five legacy JPGs retired after backup. Category manifest: assets/collection_covers/based_on/manifest.json. General Builder integration absent. | [Based on handover](artwork-categories/based-on.md) |
| Holiday and seasonal | Approved twenty-WebP release installed for commit/push; three legacy JPGs retired after backup. Category manifest: assets/collection_covers/holiday/manifest.json. Live verification pending; general Builder integration absent. | [Holiday handover](artwork-categories/holiday.md) |
| Other folders | Existing folders such as genre do not by themselves prove a category manifest or automatic builder support exists. | Inspect that category's files and consumers; document the actual state. |
