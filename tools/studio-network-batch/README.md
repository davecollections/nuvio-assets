...
The one-key follow-up has since completed. Current persistent production state contains 2,372 records: the 2,366 automatically eligible records, including `company:281730` as `expanded-threshold`, plus six exact owner-approved explicit anime-studio additions. No deferred newly eligible automatic record remains.
...
`schemas/review-state.schema.json` defines the durable exact-output approval model, and `config/review-state.json` contains one approved record for each of the 2,372 staged covers: the original 2,366-record batch plus six exact owner-approved anime-studio additions. Each record binds identity, canonical name, publish target, output hash, byte count, 1200×675 WebP metadata, approval source, and reviewed date. The top-level approval source and timestamp describe the original batch; later incremental replacements or additions update only their exact cover entries so unrelated covers are not falsely marked as re-reviewed. `src/publish-plan.mjs` validates approvals against staged files and builds an in-memory dry publish plan. A publish transaction may consume only the explicitly approved delta, add or replace its verified files, and atomically update the canonical manifest and runtime lookup while preserving every unrelated asset. Publication still requires separate explicit owner authorisation.
...
The eligibility-50 owner decisions were applied on 2026-07-14. Exactly five light-switch covers were regenerated offline; four reviewed-current covers and 2,356 unrelated outputs remained byte-for-byte and timestamp unchanged. The separately deferred `company:281730` was then generated through an exact one-key plan and reconciled as the sole absent-before addition. The final seven source-quality decisions were applied on 2026-07-15 with exactly seven offline regenerations and 2,359 unchanged outputs. The four-exception owner follow-up then regenerated exactly four covers and retained 2,362 unrelated outputs. The original final review pass changed no artwork and completed exact approval for 2,366 valid WebPs before release `studio-network-v1-2026-07-16`. Later approved additions bring current staging and exact approval state to 2,460 covers.

## Published library and maintenance

The current canonical state includes the original `studio-network-v1-2026-07-16` release, the exact 20-company `studio-network-christian-studios-supplement-2026-07-19` delta, the bounded seven-identity `studio-network-service-maintenance-2026-07-23` delta, the four-company `studio-network-compact-logo-fit-2026-07-23` replacement delta, the ABC iview correction, the six-company `studio-network-anime-studios-six-2026-08-12` addition, the provider-driven TV5 Unis Network addition, and the 87-Network `studio-network-watch-provider-batch-2026-08-16` delta with the two approved light-background corrections:

- 1,826 company covers under `assets/collection_covers/companies/{tmdb_id}.webp`;
- 660 network covers under `assets/collection_covers/networks/{tmdb_id}.webp`;
- 2,486 entries in `assets/collection_covers/manifest.json`;
- manifest SHA-256 `c2fe29424944e8f3d576ea628333e2944cae2986275de47e3b013d67a7d603cb` and byte count 6,708,800;
- combined published-asset fingerprint `2e4058dce226329545da60524703fb883096cdc27176859b233de79fcc8247e4` over 27,994,426 landscape WebP bytes plus the 660-network poster inventory.

The original publication copied exact approved staged bytes into a temporary release tree, fully validated that tree, installed and fully validated every permanent cover, and only then atomically installed the canonical manifest. Its release evidence remains under `.work/publication-release/studio-network-v1-2026-07-16/`. The supplement used the same renderer and production preset in an isolated ignored workspace, preserved all 2,366 original records value-for-value, and added only the 20 reviewed company identities. The service maintenance delta added six exact-ID logo covers and replaced only `company:6760` with its owner-approved deterministic Inter fallback. The compact-logo fit delta regenerated only `company:4`, `company:21`, `company:174`, and `company:2785` at exact byte parity with their approved Alternative B proofs. The anime-studio transaction added exactly six approved company files, changed no existing public asset, preserved 7,530 unrelated files, and verified all 5,930 runtime assets. The Network 3664 transaction added exactly its landscape and required poster, changed no existing public asset, preserved 7,536 unrelated files, and verified all 5,932 runtime assets. The 87-Network provider transaction added 174 files, replaced only the approved `network:7774` and `network:8020` landscapes, preserved 7,536 unrelated files, and verified all 6,106 runtime assets. Evidence remains under `.work/plans/anime-studios-six-2026-08-12/`, `.work/plans/watch-provider-network-3664-2026-08-16/`, and `.work/plans/watch-provider-network-batch-2026-08-16/`.
