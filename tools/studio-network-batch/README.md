...
The 153-record package was owner-reviewed on 2026-07-15. Exactly 146 `unexpectedly-opaque-source-background` reasons were resolved against the current output and source hashes; exactly seven remain `needs-better-source`. Full review preparation now contains 244 pending records and 287 unresolved reasons, with zero hash mismatch or stale resolution. The final seven-record sheet, JSON, and notes are under `.work/review-focus/opaque-final-needs-source/`. Before/after snapshots prove all 2,366 staged covers retained identical bytes and modification times. No cover was generated or regenerated, and nothing was published.
...
For a genuinely absent-before production addition, use `--added-ids` instead of `--changed-ids`. The verifier requires the path to be absent from the before snapshot and present afterward, keeps the background-review resolution requirement limited to changed/retained review decisions, and reports old paths separately from new additions.

The eligibility-50 owner decisions were applied on 2026-07-14. Exactly five light-switch covers were regenerated offline; four reviewed-current covers and 2,356 unrelated outputs remained byte-for-byte and timestamp unchanged. The separately deferred `company:281730` was then generated through an exact one-key plan and reconciled as the sole absent-before addition. The final nine-record contrast sheet is `.work/contact-sheets/production-v1/review/eligibility-50-contrast-approved.png`; the historical 76-case eligibility subset remains under `.work/contact-sheets/production-v1/review/eligibility-50-opaque-pending/`; and the reviewed all-current 153-record opaque package is under `.work/review-focus/opaque-final/`. The current full state contains 2,366 valid 1200×675 WebPs, 244 pending review entries, and zero review/output hash mismatches. Nothing has been published.
