...
test("production reason resolutions load all reviewed hash bindings", async () => {
  ...
  assert.equal(loaded.resolutions.length, 802);
  assert.deepEqual(Object.fromEntries(loaded.groups.map((group) => [group.reason, group.bindings.length])), {
    "close-background-scores": 84,
    "high-upscale-factor": 13,
    "likely-low-resolution-source": 5,
    "low-robust-contrast": 1,
    "missing-logo-text-fallback": 478,
    "unexpectedly-opaque-source-background": 180,
    "very-close-contrast": 41,
  });
});
