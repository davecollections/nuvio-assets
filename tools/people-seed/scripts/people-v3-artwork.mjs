...
Modes (select one):
  ...
  --proof                       Generate two offline Cormorant spacing and Portrait proof runs plus review sheets
  --plan                        Plan only after a separately reviewed title-logo spacing is explicitly selected
...
async function runProof({ attemptRoot, generatedAt, runtime, fontDirectory }) {
  ...
  const titleProof = await generateTitleLogoCorrectionProof({ attemptRoot, people: context.titleLogoPeople, generatedAt, runtime, fontDirectory });
  ...
  const summary = {
    ...
    titleLogoDesignCount: titleProof.first.metadata.designCount,
    titleLogoPngCountPerRun: titleProof.first.metadata.recordCount,
    titleLogoPermanentDesignSelected: true,
    ...
    titleLogoProductionLockProofRoot: titleProof.proofRoot,
    ...
    titleLogoLineWrapReport: titleProof.lineWrapPath,
...
async function runPlan({ attemptRoot, generatedAt }) {
  ...
  assert(proofSummary.titleLogoPermanentOptionSelected === true && proofSummary.presentationManifestCandidate, "Full-generation planning remains deferred until Dave explicitly selects a title-logo spacing and a matching presentation-manifest candidate is built.");
  ...
  const presentationCandidateByteCount = (await fs.stat(proofSummary.presentationManifestCandidate)).size;
