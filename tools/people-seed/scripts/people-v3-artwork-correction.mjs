...
async function latestTitleProofReport(attemptRoot) {
  ...
  for (const entry of await fs.readdir(titleRoot, { withFileTypes: true })) {
    const match = entry.isDirectory() ? /^(replay|spacing-proof|production-lock)-attempt-([0-9]{2})$/u.exec(entry.name) : null;
    if (match) candidates.push({ path: path.join(titleRoot, entry.name, "correction-proof-report.json"), order: (match[1] === "production-lock" ? 200 : match[1] === "spacing-proof" ? 100 : 0) + Number(match[2]) });
  }
...
Modes:
  ...
  --title-proof             Validate the four-person production-locked Cormorant 60 px design and replay
