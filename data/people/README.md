...
Future presentation assets remain ungenerated and unpublished. The prepared contract keeps transparent `title-logo/{tmdb_person_id}.png` files and the single existing `people hero backdrop.jpg` in a separate additive `presentation-manifest.json`; runtime schemaVersion 2 continues to resolve only Poster and Landscape assets. The final typography direction uses the exact locked Cormorant Garamond variable-font file for both the 700-weight Person name and the 500-weight fixed word `COLLECTION`. The four-person proof in `tools/people-seed/presets/people-title-logo-cormorant-spacing-v4.json` compares only 60, 70, and 80 px of clear space measured from the visible Person-name bottom; no permanent spacing is selected. No graphic element is part of the title-logo contract. The rejected Limelight proof remains only in ignored historical evidence. Any exceptional line break must be exact-ID-bound and schema-valid in `data/people/title-logo-line-break-overrides.json`.
