...
Future presentation assets remain ungenerated and unpublished. The prepared contract keeps transparent `title-logo/{tmdb_person_id}.png` files and the single existing `people hero backdrop.jpg` in a separate additive `presentation-manifest.json`; runtime schemaVersion 2 continues to resolve only Poster and Landscape assets. The production-locked `people-title-logo-cormorant-production-v1` preset uses the exact locked Cormorant Garamond variable-font file for both the 700-weight Person name and the 500-weight fixed word `COLLECTION`, with COLLECTION at 54 px, 6 px tracking and exactly 60 px of clear space measured from the visible Person-name bottom. It contains one design, uses a transparent 1863×673 canvas and has no graphic element. The rejected Limelight and 70/80 px proofs remain only in ignored historical evidence. Any exceptional line break must be exact-ID-bound and schema-valid in `data/people/title-logo-line-break-overrides.json`.
